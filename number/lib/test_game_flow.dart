...
class TestGameFlow {
  ...
  static Future<void> _runTestRounds() async {
    ...
    for (int round = 1; round <= totalRounds; round++) {
      ...
      final players = playersData.map((data) => PlayerModel.fromMap(data)).toList();
      ...
      if (isComplete) {
        ...
        if (winner != null) {
          print('🏆 Winner: Player ${winner['player_id']} with guess ${winner['guessed_number']}');
          
          // Update score
          await ScoreController.updatePlayerScore(winner['player_id'], marksPerWin);
  ...
  static Future<void> _simulateGuessing(PlayerModel player, int rangeLimit) async {
    final random = Random();
    final targetNumber = GameController.getCurrentNumber()!;
    int attempts = 0;
    const maxAttempts = 5;
...
/// Extension to add Random import
class Random {
