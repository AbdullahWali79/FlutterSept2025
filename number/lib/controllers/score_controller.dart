...
class ScoreController {
  ...
  static Future<void> updatePlayerScore(int playerId, int marksPerWin) async {
    try {
      ...
      print('ScoreController: Updated score for player $playerId: $currentScore -> $newScore (+$marksPerWin)');
    } catch (e) {
  ...
  static Future<List<Map<String, dynamic>>> getLeaderboard() async {
    try {
      ...
      print('ScoreController: Retrieved leaderboard with ${players.length} players');
      return players;
    } catch (e) {
  ...
  static Future<Map<String, dynamic>?> getWinner() async {
    try {
      ...
      leaderboard.sort((a, b) => (b['total_score'] ?? 0).compareTo(a['total_score'] ?? 0));
      final winner = leaderboard.first;

      print('ScoreController: Winner: ${winner['name']} with score ${winner['total_score']}');
      return winner;
    } catch (e) {
  ...
  static Future<List<Map<String, dynamic>>> getPlayerHistory(int playerId) async {
    try {
      final guesses = await _dbHelper.getGuessesForPlayer(playerId);
      print('ScoreController: Retrieved ${guesses.length} guesses for player $playerId');
      return guesses;
    } catch (e) {
